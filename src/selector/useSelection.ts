import { type EventHandler, type SyntheticEvent, useState } from "react"
...
const deselectParentRecursive = (map: SelectedTypeMap, todoNode: TodoNode) => {
	if (todoNode.parent === undefined) return
	const { parent } = todoNode
...
export const useSelection = (
	...
	const createHandler: SelectionHandlerCreator =
		(todoNode: TodoNode) => (e) => {
			e.stopPropagation()
	...
	const createHandlerForPreview: SelectionHandlerCreator =
		(todoNode: TodoNode) => (e) => {
			e.stopPropagation()
			deselect(todoNode)
		}
